...
class Conv_original(nn.Module):
    def __init__(self, net_width=8):
        ...
        self.linear = LinearStacked_2(3 * 32 * 32, 10,Fuse )
    def forward(self, x):
        x = x.view(x.size(0), -1)  # flatten
        x = self.linear(x)
        # return x

class myConv_flexfuse(nn.Module):
    def __init__(self, net_width=8):
        super(myConv_flexfuse, self).__init__()
        self.linear = LinearStacked_2(3 * 32 * 32, 10,Fuse )
    def forward(self, x,criterion, target):
        ...
        bmm_bwd(x, self.linear.weight, grad_output)
...
if __name__ == "__main__":
    ...
    model2 = myConv_flexfuse(32).to("cuda")
    if flag =='conv':
        model = model1
    else:
        model = model2
    ...
    # torch.save(model.state_dict(), 'model_test_doublebwd_bmm.pt')
    # exit()
    pretrained_dict = torch.load("model_test_doublebwd_bmm.pt")
