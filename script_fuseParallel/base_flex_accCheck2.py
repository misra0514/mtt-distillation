# 11.20
# FELX test 写不下去了。感觉二阶梯度还是有问题。现在得一个一个kernel去查。

# 11.19
# 最终的大测试。写一下真正实现flex fuse 的model（fwd*2， bwd*1）
# FLEX FUSE 应该和bwd+double bwd 分开单独做 / 做一个double bwd  分别比较性能。 

# 这个版本说明了stacked_bwd 一阶没问题。（有一点小误差。）
# conv的w没有区别。linear w差的比较多。
import torch 
...
class Conv_Flexfuse(nn.Module):
    ...
    def forward(self, x_conv):
        ...
        x_norm = self.conv1(x_conv)          
        x_pool = self.norm1(x_norm)          
        x_lin = self.pool1(x_pool)         
        x_lin = x_lin.view(2048,-1) 
        x_out = self.linear(x_lin)    # N x 10
        # print(x_out.shape)
        return  [x_conv,x_norm, x_pool,x_lin,x_out]
...
if __name__ == "__main__":
    ...
    for step in range(test_iter):
        ...
        if flag =='Conv_Flexfuse_flex':
            x_conv,x_norm, x_pool,x_lin,x_out = model(x)  # forward+1stbwd+weight op
            ...
            dx_conv_d1 , dconv_w ,dconv_b ,dnorm_w ,dnorm_b = ConvBlock_bwd2_1(x_conv, x_norm, x_pool, model.conv1.weight, model.norm1.weight, dx_lin_d1 ,Fuse=Fuse)



            dw = [dconv_w,dconv_b, dnorm_w,dnorm_b, dlin_w,dlin_b]
            # dw = [d.requires_grad_() for d in dw] # 因为上面没开cg=t。所以默认是不要梯度的
            grand_loss = sum((d**2).sum() for d in dw)
            print("CKPT LIN",dconv_w.sum().item())
            print("---Dbwd-GRANDLOSS2-----", grand_loss.item()) 
            print("DDNORM W flewxFuse", dnorm_w.sum().item() )
            ...
            #     print("----GRAD-----", dx_conv_d1.sum().item()) # 0.06954991072416306
            ...
            print(dw[0].sum().item())
            # [print(d.shape) for d in dw]
            # print("DDNORM W flewacc", dw[2].sum().item() )
