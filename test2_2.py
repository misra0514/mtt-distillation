# 8.15
#  把原来的测试挪到HPO上。
#  现在梯度有点对应不上： -6.866894636914367e-06 vs-8.669758244650438e-06 （step10）
...
if __name__ == "__main__":
    ...
    # torch.save(model.state_dict(), 'model_test2_1.pt')
    ...
    pretrained_dict = torch.load("model_test2_1.pt")
    ...
    img_q, label_q = cifar10[1]
    x_val = img_q.unsqueeze(0).repeat(batch_size, 1, 1, 1).to("cuda")     # 不对 x_val 求导
    target_val = torch.tensor([label_q] * batch_size, device="cuda")

    w0 = torch.cat([p.data.to('cuda').reshape(-1) for p in pretrained_dict.values()], 0).requires_grad_(True)

    criterion = nn.CrossEntropyLoss()
    # optimizer = torch.optim.SGD([w0], lr=1e-1)
    raw_log_lambda = torch.tensor(-6.9, device="cuda", requires_grad=True)  # ~ 1e-3 起始
    outer_opt = torch.optim.Adam([raw_log_lambda], lr=1e-2)
    ...
    for step in range(1):
        ...
        # 重新取内层起点，确保不把梯度传回 w0（HPO 只更新 λ）
        params = [w0]  # list of flat parameter vectors

        # 内层 K 步显式展开（unrolled），训练损失含 L2(λ)
        for _ in range(inner_steps):
            w_curr = params[-1]
            lam = torch.exp(raw_log_lambda)                       # λ > 0
            logits_s = model(x, flat_param=w_curr)
            ce = criterion(logits_s, target)
            # L2 正则（可改为 mean() 缩放；这里 sum() 更“原始”，更能显出 λ 的作用）
            l2 = 0.5 * lam * (w_curr.pow(2).sum())
            loss_s = ce + l2

            # 显式法的关键：对 w_curr 求梯度时 create_graph=True，才能把 d w / d λ 也纳入图
            grad_w = torch.autograd.grad(loss_s, w_curr, create_graph=True)[0]
            # w_next = w_curr - inner_lr * grad_w
            w_next = (w_curr - inner_lr * grad_w).detach().requires_grad_()
            params.append(w_next)

        # 外层：用适应后权重评估验证集，不含正则（典型做法）
        w_adapt = params[-1]
        logits_q = model(x_val, flat_param=w_adapt)
        grand_loss = criterion(logits_q, target_val)
        print("meta:", float(grand_loss.item()))

        # # 1  反传得到 d meta_loss / d λ（通过整个内层展开传回）
        # grand_loss.backward()
        # print("GRAD", float(raw_log_lambda.grad.item()))

        # 2 TODO: tesla method
        grad_lambda = torch.zeros_like(raw_log_lambda)
        dLdtheta=torch.autograd.grad(grand_loss,params[-1])[0]
        grad_lambda += torch.autograd.grad(params[-1],raw_log_lambda , grad_outputs=dLdtheta)[0]

        for i in range(inner_steps):
            index = inner_steps-i-1
            w = params[index]
            temp_lam = raw_log_lambda.detach().requires_grad_()
            lam = torch.exp(temp_lam)            
            logits_s = model(x, flat_param=w)
            ce = criterion(logits_s, target)
            loss_s = ce + 0.5 * lam * (w.pow(2).sum())
            grad = torch.autograd.grad(loss_s, w , create_graph=True)[0]

            gi = torch.autograd.grad(grad*inner_lr, temp_lam, grad_outputs=dLdtheta )[0]
            grad_lambda-=gi
            # grad_w0+=dLdtheta
        print("GRAD2", grad_lambda.sum().item())


        # with torch.no_grad():
        #     print("λ =", float(torch.exp(raw_log_lambda)))
        outer_opt.step()
