...
class MyNormReluFused_SndOrder(torch.autograd.Function):
    ...
    def backward(ctx, grad_grad_input, grad_grad_w, grad_grad_b):
        ...
        # print("grad_grad_input",grad_grad_input.sum().item())
        # print("grad_grad_b",grad_grad_b.sum().item())
        # print("x",input.sum().item())
        # print("dLdy",dLdy.sum().item())
...
class MyNormReluFused_FstOrder(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias):
        out = F.relu(F.batch_norm(input, running_mean=None, running_var=None,weight= weight, bias = bias, training=True))
...
def BasicBlock_double_bwd(x_conv1, x_bn1, x_conv2, x_bn2 , out,\
    ...
    print("ddx_conv2",(ddx_conv2**2).sum().item())
...
if __name__ == "__main__":
    ...
    # torch.save(model.state_dict(), 'model_test_basicblock_simple.pt')
    # exit()
    ...
    for step in range(test_iter):
        ...
        if flag =='original':
            x_out, x_fc, x_pool, x_block, x_bn = model(x)  # forward
            ...
            dw = torch.torch.autograd.grad(loss, list(model.parameters()), create_graph=True, allow_unused=True )
            # print(len(dw))
            ...
            weight = [d.sum() if d is not None else 0 for d in dw]
            ...
            with torch.no_grad():
                ...
                dx_pool[x_block <= 0] = 0
                dx_bn,dbnw, dbnb = batchNorm2d_backward(x_bn, model.bn.weight, grad_output=dx_pool)
