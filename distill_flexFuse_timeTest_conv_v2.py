# 8.4 V2 版本。目的是把bwd1-2 和bwd2-1 合并。这一版的话目前可能只是测时间用。
# 不用做训练

# 但是在此之前。可能有必要做一个F，B，W，H的代码。看看性能？

# Update: 考虑到拷贝的位置不论怎样都需要重新拷贝。这里先准备测试一下拷贝这个操作对性能的影戏到底有多少。
...
from utils_flex import build_global_group_mask, fuse_params_with_mask,recover_params,set_random_seed

from utils_flex import split_half_snd_dim_countious as split_half_snd_dim
