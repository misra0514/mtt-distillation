...
if __name__ == "__main__":
    ...
    w0 = torch.cat([p.data.to('cuda').reshape(-1) for p in pretrained_dict.values()], 0).requires_grad_()
    target_params= w0.detach()
    ...
    for step in range(1):
        ...
        for _ in range(inner_steps):
            ...
            grad_w = torch.autograd.grad(ce, w_curr, create_graph=True)[0]
        ...
        param_loss = torch.tensor(0.0).to('cuda')
        param_dist = torch.tensor(0.0).to('cuda')
        param_loss += torch.nn.functional.mse_loss(params[-1], target_params, reduction="sum")
        param_dist += torch.nn.functional.mse_loss(w0, target_params, reduction="sum")

        param_loss /= param_dist
        grand_loss = param_loss
        print("grand_loss:", float(grand_loss.item()))

        # # 1  反传得到 d meta_loss / d λ（通过整个内层展开传回）
        grand_loss.backward()
        print("GRAD", float(x.grad.sum.item()))
