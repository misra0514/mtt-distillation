...
def main(args):
    ...
    if mem_profile_enabled:
        ...
        # Start memory history once to cover model init + training loop
        torch.cuda.memory._record_memory_history(
            enabled="all",  # string keeps us on the non-legacy path
            context="all",
            stacks="all",
            max_entries=100000,
        )
    ...
    for it in range(0, args.Iteration+1):
        ...
        param_dist_list = []
        indices_chunks = []
        fwd_marker_mb = 8   # visibility marker for FWD
        bwd_marker_mb = 50  # visibility marker for BWD
        dbwd_marker_mb = 75 # visibility marker for Double BWD
        fwd_marker_elems = (fwd_marker_mb * 1024 * 1024) // 2   # float16 -> 2 bytes
        bwd_marker_elems = (bwd_marker_mb * 1024 * 1024) // 2
        dbwd_marker_elems = (dbwd_marker_mb * 1024 * 1024) // 2
        ...
        profiler_ctx = profile(
            ...
            profile_memory=True,
            with_stack=True,
        ) if mem_profile_enabled else nullcontext()

        prof = None
        with profiler_ctx as maybe_prof:
            prof = maybe_prof
            with record_function("synthetic_steps_and_backward"):
                fwd_marker = torch.empty(fwd_marker_elems, device=args.device, dtype=torch.float16)
                syn_start = time.time()
                for step in range(args.syn_steps):
                    ...
                    with record_function("BWD"):
                        if step == 0:
                            bwd_marker = torch.empty(bwd_marker_elems, device=args.device, dtype=torch.float16)
                        grad = torch.autograd.grad(ce_loss, student_params[-1], create_graph=True)[0]
                        if step == 0:
                            del bwd_marker
                            # torch.cuda.synchronize()  # ensure marker freed before Double BWD
                    student_params.append(student_params[-1] - syn_lr * grad)

                del fwd_marker
                syn_end = time.time()

                with record_function("Double BWD"):
                    dbwd_marker = torch.empty(dbwd_marker_elems, device=args.device, dtype=torch.float16)
                    ...
                    with record_function("grand_loss_backward"):
                        grand_loss.backward()
                    del dbwd_marker

        if mem_profile_enabled and prof is not None:
            snapshot_path = os.path.join(args.mem_snapshot_dir, f"mem_snapshot_it{it}_classified_temp.pickle")
            print(prof.key_averages().table(sort_by="self_cuda_memory_usage", row_limit=20))
            torch.cuda.memory._dump_snapshot(snapshot_path)
            trace_path = os.path.join(args.mem_snapshot_dir, f"trace_it{it}.json")
            try:
                prof.export_chrome_trace(trace_path)
            except Exception as e:
                print(f"Failed to export chrome trace: {e}")

            events = [e for e in prof.events() if getattr(e, "self_cuda_memory_usage", 0) > 0]
            if events:
                events.sort(key=lambda e: e.self_cuda_memory_usage, reverse=True)
                top = events[0]
                mb = top.self_cuda_memory_usage / 1024 / 1024
                print(f"\n[MemPeak] {top.name} allocated ~{mb:.1f} MB")
                if getattr(top, "stack", None):
                    print("Stack (most recent last):")
                    for frame in top.stack:
                        print("  ", frame)
