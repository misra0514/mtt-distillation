# 8.13
# 开一个新的系列，用来测试 MAML 的模式是否可以用原来的CKPT方法，再进而更改成并行的模式。
# 可能需要依次测试tesla_loss 和 ckpt
# 在step=1测试基本成功
# MAML并不具备并行特性。因为dgrad2/dw0 需要计算dw1/dw0 
# 本质上感觉是因为w0在后续的几个step里面都没有用上，导致每一层不存在局部梯度，必须反向传播到最后（还好MAML还是可以做数据并行的）
...
if __name__ == "__main__":
    ...
    for step in range(1):
        ...
        # ====== 内层：从 w0 出发做 K 步更新，输入一直用 x ======
        # w_curr = w0
        for _ in range(inner_steps):
            forward_params = params[-1]
            out_s = model(x, flat_param=forward_params)
            loss_s = criterion(out_s, target)
            # MAML 的关键：需要高阶梯度，所以 create_graph=True
            grad = torch.autograd.grad(loss_s, forward_params, create_graph=True)[0]
            # new = forward_params - inner_lr * grad
            new = forward_params - (inner_lr * grad).detach().clone()
            params.append(new )
            # print("inner:", float(loss_s.item()))

        w_adapt = params[-1]
        # ====== 外层：用适应后的参数在 x_val 上算 meta loss ======
        out_q = model(x_val, flat_param=w_adapt)
        meta_loss = criterion(out_q, target_val)
        print( "meta:", float(meta_loss.item()))

        # # 1 gradLoss.backward
        # meta_loss.backward()
        # print("GRAD", w0.grad.sum().item())

        # 2 TODO: tesla method: dLdtheta 其实就是个一阶导数，本质上类似dw，外层不需要lr，也不包含
        grad_w0 = torch.zeros_like(w0)
        dLdtheta=torch.autograd.grad(meta_loss,params[-1])[0]
        grad_w0+=dLdtheta

        # dw = torch.autograd.grad(grad, w0, grad_outputs=dLdtheta*inner_lr)[0]
        # dw=dLdtheta-dw
        # print("GRAD2", dw.sum().item())
        gi=None

        for i in range(inner_steps):
            index = inner_steps-i-1
            # grad_i =( inner_lr*grad) 
            # print(grad.shape)
            # print(dLdtheta.shape)
            # w=dLdtheta@grad_i
            # 1 get grad 
            w = params[index]
            print(index)
            out = model(x, flat_param=w)
            loss_s = criterion(out, target)
            grad = torch.autograd.grad(loss_s, w, create_graph=True)[0]

            # if(gi is not None):
            #     gi = torch.autograd.grad(grad, w0,grad_outputs=gi*inner_lr)[0]
            # else:
            gi = torch.autograd.grad((dLdtheta@ grad*inner_lr), w0)[0]
            grad_w0-=gi
            # grad_w0+=dLdtheta
        print("GRAD2", grad_w0.sum().item())
