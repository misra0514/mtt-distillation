...
def _instancenorm_backward_kernel(
    dY, X, gamma, mean_ptr, rstd_ptr, dX, dgamma, dbeta,
    stride_n, stride_c, stride_hw, C, HW, BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(0)
    batch_id = pid // C
    c = pid % C
    ...
    for off in range(0, HW, BLOCK_SIZE):
        ...
        dy_val = tl.load(dy + idx, mask=mask, other=0.).to(tl.float32)
        x_hat = (x_val - mean) * rstd
        # 来自 LayerNorm 的导数公式 adapted
        N = HW
        # term1 = dy_val * gamma[c]
        g = tl.load(gamma + c)
        term1 = dy_val * g
...
def instance_norm_backward_triton(x, gamma, grad_output, eps=1e-5):
...
def instance_norm_double_backward_kernel(
    x_ptr, gO_ptr, ggX_ptr,
    gamma_ptr, beta_ptr,     # gamma_ptr 用于 γ，beta_ptr 保留但不使用
    ...
    N, C, M,
    eps: tl.constexpr,
    has_gamma: tl.constexpr, has_beta: tl.constexpr,
...
def instanceNorm_double_backwards_triton(x, gamma, beta, ggX, ggG, ggB, gO, eps=1e-5):
    N, C, H, W = x.shape
    M = H * W
    x_flat = x.view(N, C, M)
    ...
    grid = (N * C,)
        ...
        # beta if beta is not None else torch.empty(1, device=x.device),
    instance_norm_double_backward_kernel[grid](
        x_flat, gO_flat, ggX_flat,
        gamma if gamma is not None else torch.empty(1, device=x.device),
        beta  if beta  is not None else torch.empty(1, device=x.device),
...
def instanceNorm_double_backwards_fn_cln(x, gamma, beta, ggX, ggG, ggB, gO,
                                     eps=1e-5, training=True):
    N, C, H, W = x.shape
    M = H * W
...
class Snd_Order_MyLinearFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, dLdy, input, weight, bias):
        ctx.save_for_backward( input, weight, bias,dLdy )

        grad_output, dw, db = instance_norm_backward_triton(input, weight, dLdy)
    ...
    def backward(ctx, grad_grad_input, grad_grad_w, grad_grad_b):
        input, weight, bias, dLdy = ctx.saved_tensors
        gx, gG, ggO = instanceNorm_double_backwards_triton(input, weight, bias, grad_grad_input, grad_grad_w,grad_grad_b, dLdy,1e-5)
        # gx, gG, ggO = instanceNorm_double_backwards_fn(input, weight, bias, grad_grad_input, grad_grad_w,grad_grad_b, dLdy,1e-5,True   )
...
class MyLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias):
        ...
        out = F.instance_norm(input,weight= weight, bias = bias)
        return  out
    @staticmethod
    def backward(ctx, dLdy):
        input, weight, bias = ctx.saved_tensors
        return Snd_Order_MyLinearFunction.apply(dLdy ,input, weight, bias)
        # db = gin.sum(0)
