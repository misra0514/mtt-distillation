# 2.19 从sliding window上面merge 来的，并且匹配了基本的acctest参数。
# 测试了一下发现开不开ckpt，其实梯度是不太一样的，小数点后5位左右有区别。（并且很奇怪的点在于，只要开了cg，梯度就正好能对齐了）
# 考虑到另一个branch上的收敛测试都过了，暂时认为是误差吧
...
def main(args):
    ...
    for it in range(0, args.Iteration+1):
        ...
        # grand_loss.backward()
        # 如果对x求，那就是最后一个sync。如果是synimage那就是整个图。
