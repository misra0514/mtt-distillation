...
def main(args):
    ...
    for it in range(0, args.Iteration+1):
        ...
        for step in range(args.syn_steps):
            ...
            # print(this_y.shape)
            # print(out.shape)
            this_y = this_y.repeat(int(args.Constk))  
        ...
        target_params_all = torch.cat([p.data.to(args.device).reshape(-1) for p in target_params  for _ in range(int(args.Constk))], 0).requires_grad_(True)
