...
class MultiHeadSelfAttention_Fused(nn.Module):
    ...
    def run_double_bwd(
        ...
        dd_dqkv_linear, dx_d2, _ = grouped_linear_double_bwd(
            x=x,
            w=self.qkv.weight,
            grad_output=dqkv_linear,
...
class TransformerBlock_Fused(nn.Module):
    ...
    def run_first_bwd(module, tape, grad_output, Fuse=None):
        ...
        d_activates = {
            ...
            "dx_fc1": dx_fc1,
            "dx_norm2": dx_norm2,
            "dx_res1_from_norm2": dx_res1_from_norm2,
            "dx_res1_total": dx_res1_total,
            # attention side
            "dx_attn": dx_attn,
            "dx_norm1": dx_norm1,
            "dx_in_from_norm1": dx_in_from_norm1,
            "dx_in_from_skip": dx_in_from_skip,
    ...
    def run_double_bwd(
        ...
        # --------------------------------------------------
        # unpack first-bwd activations
        # --------------------------------------------------
        dx_norm1 = d_activates["dx_norm1"]
        dx_norm2 = d_activates["dx_norm2"]
        dx_fc1 = d_activates["dx_fc1"]
        dx_gelu = d_activates["dx_gelu"]
        dx_fc2 = d_activates["dx_fc2"]
        ...
        # ==================================================
        dx_fc1_d2, dd_dx_gelu = gelu_double_bwd(
            x=x_fc1,
            grad_output=dx_gelu,
    ...
    def run_bwd2_1(
        ...
        dx_gelu_d2 = d_activates.get("dx_gelu_d2", None)
        if dx_gelu_d2 is not None:
        ...
        dx_fc1_d2 = d_activates.get("dx_fc1_d2", None)
        ...
        dx_norm2_d2 = d_activates.get("dx_norm2_d2", None)
        ...
        #       dx_in_from_skip
        #       dx_attn
        dx_res1_d2 = d_activates.get("dx_res1_d2", None)
        if dx_res1_d2 is not None:
        ...
        # block input x_in.
        dx_in_d2 = d_activates.get("dx_in_d2", None)
        if dx_in_d2 is not None:
...
class ViT_Fused(nn.Module):
    ...
    def forward(self, x):
        ...
        # [B, Fuse, N+1, D]
        tape["patch"]["x_cat"] = x_cat
        tape["patch"]["x_pos"] = x_pos
        # 这里要求 TransformerBlock_Fused.forward 也返回 block_tape
        # x_block = self.block(x_pos)
        ...
        # [B, Fuse, num_classes]
        # 重要：保留你的原始约定。
        # 无 permute，直接 reshape。
        # 所以 target 要用 repeat_interleave(Fuse)。
    ...
    def run_first_bwd(self, tape, target, Fuse=None):
        ...
        x_head = head["x_head"]
        x_out = head["x_out"]
        # 0. target shape check
        # 你的 forward 是:
        #   x_head [B, Fuse, C] -> x_head.reshape(B*Fuse, C)
        #
        # 所以 target 必须是:
        #   target.repeat_interleave(Fuse)
        #
        # 如果外面已经 repeat_interleave 过了，这里不要再重复。
        # if target.numel() == B:
        #     target_ce = target.repeat_interleave(Fuse)
        # else:
        #     target_ce = target

        # assert target_ce.numel() == B * Fuse
        # assert x_out.shape == (B * Fuse, C)
        # 1. CE bwd
        ...
        #
        # backward:
        #   only cls position receives dx_cls
        dx_norm = torch.zeros_like(x_norm)
        dx_norm[:, :, 0, :] = dx_cls
        # [B, Fuse, N+1, D]
        dx_block, dnormw, dnormb = grouped_layernorm_backward(
            x_norm_in,
            self.norm.weight,
            grad_output=dx_norm,
            Fuse=Fuse,
        ...
        dx_pos, d_block_activates, d_block_weights, d_block_weights_all = self.block.run_first_bwd(
            block_tape,
        ...
        # dx_cat: [B, Fuse, N+1, D]
        # forward:
        #   x_cat = cat(cls_token_expand, x_tokens, dim=2)
        ...
        # [B, Fuse*D, Hp, Wp]
        dx_in, dpatchw, _ = conv_bwd(
            x_in,
            self.patch_embed.weight,
            grad_output=dx_patch,
            stride=self.patch_embed.stride[0],
            padding=self.patch_embed.padding[0],
            groups=Fuse,
        )
        ...
        d_weights_list_all = []
        # 注意顺序最好和 model.parameters() 尽量一致：
        # patch_embed, cls_token, pos_embed, block, norm, head
    ...
    def run_double_bwd(
        ...
        这个函数只做一件事：
            沿着 first-bwd graph 反向传播 cotangent，
            并把 activation-level d2 contribution 写入 d_activates。
        ...
        返回:
            dd_dx_out:
                cotangent wrt CE first-bwd output dx_out。
        ...
        dx_out = d_activates["dx_out"]
        dx_head = d_activates["dx_head"]
        dx_norm = d_activates["dx_norm"]
        dx_patch = d_activates["dx_patch"]
        ...
        assert dx_norm.shape == head["x_norm"].shape
        ...
        dd_dx_patch, x_in_d2, _ = conv_double_bwd(
            ddgrad_in,
            ddpatchw,
            ddpatchb,
            dx_patch,
            self.patch_embed.weight,
            x_in,
            stride_=list(self.patch_embed.stride),
            padding_=list(self.patch_embed.padding),
            groups_=Fuse,
        )
        # dd_dx_patch: [B, Fuse*D, Hp, Wp]
        # x_in_d2:     [B, C*Fuse, H, W]
    ...
    def run_bwd2_1(
        ...
        输入:
            dd_dx_out:
                run_double_bwd 返回的 cotangent wrt CE first-bwd output dx_out。

        返回:
            dx_in:
                d grand_loss / d input。
        """
        if Fuse is None:
            Fuse = self.Fuse
        ...
        x_cls_d2 = d_activates.get("x_cls_d2", None)
        if x_cls_d2 is not None:
        ...
        x_in_d2 = d_activates.get("x_in_d2", None)
        if x_in_d2 is not None:
